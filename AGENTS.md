...
- Preserve explicit NumPy cosine retrieval, stable-id tie-breaking, top-2 context, citation metadata, verbatim prompt capture, deeply immutable evaluation captures, validated transactional index generations, and exact abstention unless the task intentionally changes those contracts.
...
Any command that can contact OpenAI or Anthropic requires explicit approval for that exact invocation. This includes paid pytest tiers, `pipeline.py ingest`, `pipeline.py ask`, and live use of `ask()`. Approval does not carry to a manually initiated retry, another command, or another tier. Before requesting approval, state the exact command, providers and models, selected cases, the maximum OpenAI SDK attempt fan-out under `max_retries=0`, the 120-second per-network-operation OpenAI timeout and its timeout/cost uncertainty, and separate RagaliQ/Anthropic retry uncertainty. The RagaliQ cost limit is an approximate post-test guard, not a hard pre-spend cap. Never print, persist, or commit provider keys. Do not change the OpenAI retry or timeout policy without explicit approval. Use `.agents/skills/run-paid-evaluation/SKILL.md` for paid work.
...
- Flag any path that could execute OpenAI or Anthropic calls without an explicit paid marker and per-run approval, and any VerdigrisE-owned OpenAI client that bypasses `max_retries=0` or the 120-second timeout.
