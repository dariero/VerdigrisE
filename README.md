...
| Vectorized top-`k` selection instead of a total sort | `pipeline.py:417-420` sorts by `(-similarity, id)`, and expressing that exact documented tie-break as a readable key is worth more here than selection speed | The key is interpreted Python called once per row, and all `n` rows are ordered to return `TOP_K`, which is 2. This is the operation that ends the design, not the vector math | Measured on a step-2 grid, five trials each: the sort first costs at least the matrix product in `12 < n <= 14` at `d` = 12 and at `d` = 128, and in `30 < n <= 32` at `d` = 1536. All of that sits inside the overhead-dominated regime below, and none of it is a size at which anything fails. Where it matters is at scale: at `n` = 100,000, `d` = 1536 the sort is about 52 ms against about 20 ms for the matrix product |
| Approximate nearest neighbour instead of exact search | Exact search has recall 1.0 and no index-build step or tuning parameters to explain | `pipeline.py:416` touches every row on every query. Latency is linear in `n` before the sort above dominates it | Measured: at `n` = 100,000 and `d` = 128 the matrix product is about 1 ms inside a search of about 54 ms, so exact search is not the binding constraint at any size this sandbox reaches |
...
Three regimes, not one crossover. A NumPy call costs something fixed before any arithmetic happens: argument parsing, dtype and shape resolution, and BLAS dispatch. Measured at one row, that floor is about 0.0025 ms and is the same at `d` = 12, `d` = 128, and `d` = 1536, because dispatch does not care how wide the vectors are. Below roughly a thousand rows the matrix product is measuring that floor rather than the arithmetic, which is why its cost barely moves across a 128-fold increase in width: about 0.0022 ms against about 0.0033 ms at `n` = 8. By `n` = 100,000 the same comparison is about 0.17 ms against about 20 ms, a factor of roughly 120, and the arithmetic is plainly in charge.

That is why the sort-versus-matmul crossover is not a decision-relevant number. It falls between 12 and 32 rows depending on width, deep inside the regime where both operations are dwarfed by fixed overhead, and a reader who reads it as "this design fails at 14 documents" would be badly misled.

The crossover is published as a bracket rather than a point because it is located by sampling a grid. A transition reported at a sampled `n` only means it happened above the previous sample and at or below that one, so the interval, not the endpoint, is what the measurement supports. The grid step is 2 rows and the determination was repeated five times per width; the located point did not move between trials, so grid resolution rather than timing noise is what bounds it.
...
|---|---|---|---|
| 1,000 | about 0.3 ms | about 0.3 ms | about 0.4 ms |
| 10,000 | about 3.5 ms | about 3.7 ms | about 6 ms |
| 100,000 | about 53 ms | about 54 ms | about 73 ms |

Every figure above is a median of five trials at 21 repetitions each, and every one is stated to the precision those trials support and no further. Spread across trials stayed under 13% at all but one grid point; at `n` = 100,000 and `d` = 12 it reached 38%, which is why that row is given to two significant figures rather than the four the raw timer prints. Treat any comparison that turns on a round threshold as unresolved.

Read against a threshold a reader supplies: single-query search stays below a millisecond somewhere between 1,000 and 10,000 entries at every width tested, and is single-digit milliseconds at 10,000. At 100,000 entries it is tens of milliseconds, and the resident matrix at `d` = 1536 is 585.94 MiB, which is exact arithmetic on the row count and width rather than a timing, and is the one figure here that carries full precision.

One measured fact inverts the expected answer and is the one that applies today. At the size this sandbox actually runs, 8 entries at width 12, the matrix product and the sort together are about 0.0037 ms of an approximately 0.030 ms search, which is about 12%. The other 88% is query normalization plus construction of the two `RetrievedChunk` capture models. At the real corpus size neither scaling operation matters, and optimizing either would change nothing.
