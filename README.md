...
| Corpus size | Order of magnitude | `d` = 12 | `d` = 128 | `d` = 1536 |
|---|---|---|---|---|
| 8, the shipped corpus | tens of microseconds | 0.029 to 0.032 ms | 0.030 to 0.034 ms | 0.034 to 0.036 ms |
| 1,000 | a few hundred microseconds | 0.30 to 0.32 ms | 0.31 to 0.33 ms | 0.35 to 0.46 ms |
| 10,000 | single-digit milliseconds | 3.7 to 4.2 ms | 3.7 to 4.7 ms | 5.9 to 6.9 ms |
| 100,000 | tens of milliseconds | 50 to 63 ms | 51 to 65 ms | 68 to 102 ms |

Those are **observed ranges, not bounds**. Each cell is the lowest and highest value seen across nine runs of the documented command, each run itself a median of five trials at 21 repetitions. The `n` = 100,000 row comes from the opt-in invocation. Run-to-run spread reached 27% at 1,000 entries and 41% at 100,000, both at `d` = 1536, so a tenth run can land outside any of these ranges and would not contradict them.

An earlier revision published these as single values and one band, `about 0.4 to 0.6 ms` for `d` = 1536 at 1,000 entries. Nine runs put the median at 0.3623 ms, below the band entirely: a reader following the documented command got a number the section excluded. Seven of the twelve cells failed the same test. Rather than widen a band until it survives, the figures are now stated as what the measurement actually produced, which is the same standard already applied to the crossover above, retracted rather than re-bracketed once trial noise exceeded the gap being measured.

Read against a threshold a reader supplies: single-query search is tens of microseconds at the shipped corpus size, a few hundred microseconds at 1,000 entries, single-digit milliseconds at 10,000, and tens of milliseconds at 100,000, touching 102 ms at the widest vector measured. The resident matrix at `n` = 100,000 and `d` = 1536 is 585.94 MiB, which is exact arithmetic on row count and width rather than a timing, and is the one figure here that carries full precision.
