...
|---|---|---|---|
| 8, the shipped corpus | about 0.03 ms | about 0.03 ms | about 0.035 ms |
| 1,000 | about 0.3 ms | about 0.32 ms | about 0.4 to 0.6 ms |
| 10,000 | about 3.6 ms | about 3.8 ms | about 5.9 ms |
| 100,000 | about 53 ms | about 54 ms | about 73 ms |

Each figure is a median of five trials at 21 repetitions, which is what the documented command runs by default; the script prints the observed spread per grid point so the precision of every row is visible rather than asserted. Spread is usually a few percent and occasionally much worse: single trials at `n` = 512 and `n` = 1,000 have been seen to range by a factor of two or three when the machine is busy, which is why the `d` = 1536 row at 1,000 entries is given as a band and why nothing here is stated to more than two significant figures. The `n` = 100,000 row comes from the opt-in invocation. Treat any comparison that turns on a round threshold as unresolved.
