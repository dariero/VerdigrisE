...
2. Select exactly one paid command. Before requesting approval, report the exact command, selected cases, provider key names, models, nominal request fan-out, provider-retry uncertainty, and cost uncertainty without displaying credential values:
   ...
   - Selecting one parametrized pytest node from either paid tier does not reduce the OpenAI fan-out: the module-scoped `live_openai_records` fixture still creates all seven records, for the same nominal 15 OpenAI requests. A selected semantic node limits RagaliQ judging to that case, but its claim-verification fan-out remains model-output-dependent.
