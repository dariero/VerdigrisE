...
3. Record `uv --version` and run lock-generation commands through the generator version documented in `README.md`. The commands below use uv 0.11.30 explicitly. A uv-version change is separate tooling scope: update the CI pin, README generator-version statement, and skill commands together only when explicitly approved. Never delete the committed `uv.lock`.
